    ...
    <div className = 'flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      <div className = 'w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
          {relatedDoc.slice(0,5).map((item,index)=>(
            <div onClick = {() => {navigate(`/appointments/${item._id}`); scrollTo(0,0)}} className = 'border border-blue-200 rounded-xl overflow-hidden cursoSr-pointer hover:translate-y-[-10px] transition-all duration-500' key = {index}>
